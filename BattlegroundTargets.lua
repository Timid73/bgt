...
local specBuffs = BattlegroundTargets_specBuffs;
local specSpells = BattlegroundTargets_specSpells;
local healers = {};
...
function BattlegroundTargets:CheckForEnabledBracket(bracketSize)
    if (BattlegroundTargets_Options.EnableBracket[bracketSize]) then
        ...
        if (bracketSize == 15 or bracketSize == 40) then
...
function BattlegroundTargets:EnableConfigMode()
    ...
    if (not testDataLoaded) then
        ...
        ENEMY_Data[4] = { name = "Target_Dd-WoW Circle 3.3.5a x25", classToken = "HUNTER" }
        ...
        ENEMY_Data[14] = { name = "Target_Nn-WoW Circle 3.3.5a x10", classToken = "MAGE" }
        ENEMY_Data[15] = { name = "Target_Oo-WoW Circle 3.3.5a x15-20", classToken = "SHAMAN" }
        ENEMY_Data[16] = { name = "Target_Pp-WoW Circle 3.3.5a x25", classToken = "ROGUE" }
        ENEMY_Data[17] = { name = "Target_Qq-WoW Circle 3.3.5a x100", classToken = "WARLOCK" }
        ENEMY_Data[18] = { name = "Target_Rr-WoW Circle 3.3.5a x300", classToken = "PRIEST" }
        ENEMY_Data[19] = { name = "Target_Ss-WoW Circle 3.3.5a x5", classToken = "WARRIOR" }
        ENEMY_Data[20] = { name = "Target_Tt-WoW Circle 3.3.5a x10", classToken = "DRUID" }
        ENEMY_Data[21] = { name = "Target_Uu-WoW Circle 3.3.5a x15-20", classToken = "PRIEST" }
        ENEMY_Data[22] = { name = "Target_Vv-WoW Circle 3.3.5a x25", classToken = "MAGE" }
        ENEMY_Data[23] = { name = "Target_Ww-WoW Circle 3.3.5a x100", classToken = "SHAMAN" }
        ...
        ENEMY_Data[31] = { name = "Target_Ij-WoW Circle 3.3.5a x5", classToken = "HUNTER" }
        ENEMY_Data[32] = { name = "Target_Kl-WoW Circle 3.3.5a x10", classToken = "WARRIOR" }
        ENEMY_Data[33] = { name = "Target_Mn-WoW Circle 3.3.5a x15-20", classToken = "PALADIN" }
        ENEMY_Data[34] = { name = "Target_Op-WoW Circle 3.3.5a x25", classToken = "MAGE" }
        ENEMY_Data[35] = { name = "Target_Qr-WoW Circle 3.3.5a x100", classToken = "DEATHKNIGHT" }
        ENEMY_Data[36] = { name = "Target_St-WoW Circle 3.3.5a x300", classToken = "MAGE" }
        ENEMY_Data[37] = { name = "Target_Uv-WoW Circle 3.3.5a x5", classToken = "HUNTER" }
        ENEMY_Data[38] = { name = "Target_Wx-WoW Circle 3.3.5a x10", classToken = "WARLOCK" }
        ENEMY_Data[39] = { name = "Target_Yz-WoW Circle 3.3.5a x15-20", classToken = "WARLOCK" }
        ENEMY_Data[40] = { name = "Target_Zx-WoW Circle 3.3.5a x25", classToken = "ROGUE" }
...
local sortfunc13 = function(a, b) -- Класс/Имя | 13
...
local sortfunc11 = function(a, b) -- Класс/Имя | 11
...
local sortfunc12 = function(a, b) -- Класс/Имя | 12
...
local sortfunc2 = function(a, b) -- Класс/Имя | 2
    if (a.name < b.name) then
        return true;
    end
end
...
function BattlegroundTargets:MainDataUpdate()
    ...
    if (ButtonSortBy == 1) then
        if (ButtonSortDetail == 3) then
            table_sort(ENEMY_Data, sortfunc13); -- Класс/Имя | 13
        elseif (ButtonSortDetail == 1) then
            table_sort(ENEMY_Data, sortfunc11); -- Класс/Имя | 11
        else
            table_sort(ENEMY_Data, sortfunc12); -- Класс/Имя | 12
        end
    else
        table_sort(ENEMY_Data, sortfunc2); -- Имя | 2
    end
    ...
    local ButtonShowHealthBar = OPT.ButtonShowHealthBar[currentSize];
    local ButtonShowHealthText = OPT.ButtonShowHealthText[currentSize];
    local ButtonShowTarge = OPT.ButtonShowTarget[currentSize];
    local ButtonShowFocus = OPT.ButtonShowFocus[currentSize];
    local ButtonShowFlag = OPT.ButtonShowFlag[currentSize];
    local ButtonShowAssist = OPT.ButtonShowAssist[currentSize];
    local ButtonRangeCheck = OPT.ButtonRangeCheck[currentSize];
...
function BattlegroundTargets:CheckFlagCarrierEND() -- FLAGSPY
    ...
    if not OPT.ButtonShowHealthBar[currentSize] and
            not OPT.ButtonShowHealthText[currentSize] and
            (not OPT.ButtonRangeCheck[currentSize] or OPT.ButtonTypeRangeCheck[currentSize] < 2) then
        ...
        if (OPT.ButtonRangeCheck[currentSize]) then
            ...
            elseif (OPT.ButtonTypeRangeCheck[currentSize] >= 2) then
                if (ranges[playerClassEN]) then
                    ...
                    elseif playerClassEN == "MONK" and playerLevel < 14 then
                        Print("WARNING", playerClassEN, "Required level for class-spell based rangecheck is 14.");
...
function BattlegroundTargets:UNIT_AURA(event, unit)
    if (ENEMY_Names[unit.name] and not healers[unit]) then

        local index = 1

        --Buffs
        while ( true ) do
            local name, rank, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable, unitID = UnitAura(unit, index, "HELPFUL")
            if ( not name ) then break end

            -- Spec detection
            BattlegroundTargets:DetectHealer(unitID, specBuffs[name])
...
function BattlegroundTargets:DetectHealer(unitID, spec)
    ...
    SELECTED_CHAT_FRAME:AddMessage( "detecting healers" );
    if (spec == healer) then
        table.insert(healers, unitID)
        SELECTED_CHAT_FRAME:AddMessage( "healers detected with ID " .. unitID );
    end
end
